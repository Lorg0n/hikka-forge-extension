...
class ModuleManager {
    ...
    private evaluateModulesForCurrentUrl(onlyLoadMissing: boolean = false): void {
        ...
        this.moduleDefinitions.forEach(moduleDef => {
            ...
            const isUnloading = this.unloadingModules.has(moduleDef.id);
            const isActive = isComponentActive || areStylesActive || isUnloading;
    ...
    private loadModule(moduleDef: ForgeModuleDef): void {
        ...
        if (this.unloadingModules.has(moduleDef.id)) {
            ...
            const activeInstance = this.activeModuleRoots.get(moduleDef.id);
            if (activeInstance) {
                try {
                    activeInstance.root.unmount();
                    activeInstance.container.remove();
                } catch (error) {
                    console.error(`[Hikka Forge] Error cleaning up during loadModule for ${moduleDef.name}:`, error);
                }
    ...
    private unloadModule(moduleId: string): void {
        ...
        if (this.unloadingModules.has(moduleId)) {
            ...
            console.log(`[Hikka Forge] Cancelled previous unload timeout for ${name}`);
        }
    ...
    private handleUrlChange(newUrl: string): void {
        ...
        this.unloadingModules.forEach((timeout, moduleId) => {
            ...
            console.log(`[Hikka Forge] Cancelled unloading timeout for module ${moduleId}`);
            ...
            if (activeInstance) {
                try {
                    ...
                    console.error(`[Hikka Forge] Error force-unloading module ${moduleId}:`, error);
                }
                this.activeModuleRoots.delete(moduleId);
            }
        });
