...
class ModuleManager {
    ...
    private evaluateModulesForCurrentUrl(onlyLoadMissing: boolean = false): void {
        ...
        this.moduleDefinitions.forEach(moduleDef => {
            ...
            const isComponentCurrentlyInDom = this.activeModuleRoots.has(moduleDef.id);
            const areStylesCurrentlyInjected = this.activeStyleTags.has(moduleDef.id);
            const isCurrentlyUnloading = this.unloadingModules.has(moduleDef.id);
            const isCurrentlyManagedAsActiveOrUnloading = isComponentCurrentlyInDom || areStylesCurrentlyInjected || isCurrentlyUnloading;
            if (isEnabled && matchesUrl) {
                if (!isComponentCurrentlyInDom && !areStylesCurrentlyInjected && !isCurrentlyUnloading) { 
                    modulesToActivate.push(moduleDef);
                } else if (isCurrentlyUnloading) {
                    console.log(`[Hikka Forge] Module ${moduleDef.name} was unloading, but is needed for new URL. Cancelling unload and reactivating.`);
                    modulesToDeactivate.push(moduleDef.id); 
                    modulesToActivate.push(moduleDef);
                } else if (isComponentCurrentlyInDom || areStylesCurrentlyInjected) {
                    if (!onlyLoadMissing) { 
                        console.log(`[Hikka Forge] Module ${moduleDef.name} is already active but URL changed or full re-eval. Marking for re-load.`);
                        modulesToDeactivate.push(moduleDef.id);
                        modulesToActivate.push(moduleDef);
                    } else {
                        console.log(`[Hikka Forge] Module ${moduleDef.name} is already active and onlyLoadMissing is true. No action.`);
                    }
                }
            } else {
            ...
            setTimeout(() => {
                modulesToActivate.forEach(moduleDef => {
                    ...
                    if (isEnabled && matchesUrl) {
                        ...
                        console.log(`[Hikka Forge] Conditions for ${moduleDef.name} changed before delayed activation. Skipping.`);
                    }
                });
            }, 50); 
        } else {
            console.log('[Hikka Forge] No modules to activate.');
        }
    }
    ...
    private loadModule(moduleDef: ForgeModuleDef): void {
        if (this.unloadingModules.has(moduleDef.id)) {
            ...
            console.log(`[Hikka Forge] Cancelled unloading of ${moduleDef.name} because it's being loaded again.`);
            const oldInstance = this.activeModuleRoots.get(moduleDef.id);
            if (oldInstance) {
                console.log(`[Hikka Forge] Removing DOM of previously unloading module ${moduleDef.name} before reload.`);
                try {
                    oldInstance.root.unmount();
                    oldInstance.container.remove();
                } catch (error) {
                    console.error(`[Hikka Forge] Error cleaning up unloading module ${moduleDef.name} during reload:`, error);
                }
                this.activeModuleRoots.delete(moduleDef.id);
            }
        }
        const existingActiveInstance = this.activeModuleRoots.get(moduleDef.id);
        if (existingActiveInstance) {
            console.warn(`[Hikka Forge] Module ${moduleDef.name} component root already exists. Unmounting and removing before re-injection.`);
            try {
                existingActiveInstance.root.unmount();
                existingActiveInstance.container.remove();
            } catch (error) {
                console.error(`[Hikka Forge] Error removing existing active component for ${moduleDef.name}:`, error);
            }
            this.activeModuleRoots.delete(moduleDef.id);
        }
        if (moduleDef.styles) {
        ...
        } else if (!moduleDef.styles && !moduleDef.component) {
            console.log(`[Hikka Forge] Module ${moduleDef.name} has no styles or component to load.`);
        }
    }
    ...
    private unloadModule(moduleId: string): void {
        ...
        if (this.unloadingModules.has(moduleId)) {
            ...
            console.log(`[Hikka Forge] Cleared existing unload timeout for ${name}.`);
        }
        ...
        if (activeInstance) {
            ...
            console.log(`[Hikka Forge] Unload process initiated for ${name} component. DOM removal delayed.`);
        } else {
            this.activeModuleRoots.delete(moduleId); 
            this.unloadingModules.delete(moduleId); 
            console.log(`[Hikka Forge] No active component instance for ${name} to animate out. Ensuring it's marked as unloaded.`);
        }
    ...
    private handleUrlChange(newUrl: string): void {
        if (newUrl === this.currentUrl) {
            console.log(`[Hikka Forge] URL change to ${newUrl} is same as current. Ignoring.`);
            return;
        }
        ...
        this.unloadingModules.forEach((timeout, moduleId) => {
            ...
            console.log(`[Hikka Forge] Cancelled pending unload animation for module ${moduleName} due to URL change.`);
            ...
            if (activeComponent) {
                try {
                    ...
                    console.error(`[Hikka Forge] Error force-removing component for ${moduleName} during URL change cleanup:`, error);
                }
                this.activeModuleRoots.delete(moduleId);
            }
        });
